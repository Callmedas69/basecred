...
Agent self-registration. No authentication required. Rate limited to 10 requests/hour per IP.
...
| `telegramId` | `string` | Yes | Owner's Telegram handle (max 128 chars) |
...
Owner submits a tweet URL containing the verification code. Rate limited: 5 attempts/min per IP and per claimId.
...
Checks the agent's owner's reputation across all 5 contexts. Requires API key authentication. No request body needed — the owner's wallet is derived from the API key.
...
| 504 | `CHECK_OWNER_ERROR` | Request timed out (45s safeguard) |
...
Public endpoint. Returns the most recent agent reputation check entries. No authentication required.
...
The `ownerAddress` is truncated for privacy (first 6 + last 4 characters).
...
```
1. POST /api/v1/agent/register          → Get apiKey + claimUrl
2. Send claimUrl to owner               → Owner posts tweet
3. GET  .../register/{claimId}/status    → Poll until "verified"
4. POST /api/v1/agent/check-owner       → Get reputation results
