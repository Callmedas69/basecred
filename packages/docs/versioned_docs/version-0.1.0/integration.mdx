...
Evaluates a subject's reputation for a given context and returns a decision with explanation. Rate limited: 100 requests/minute per API key (when using `x-api-key` header).
...
| `signalCoverage` | `number` | Percentage of signals fetched (0–1) |
...
| 429 | `RATE_LIMITED` | Too many requests per API key |
...
const checkReputation = async (address: string): Promise<DecisionResponse> => {
  const response = await fetch("https://www.zkbasecred.xyz/api/v1/decide", {
    method: "POST",
...
def check_reputation(address: str) -> dict:
    response = requests.post(
        "https://www.zkbasecred.xyz/api/v1/decide",
