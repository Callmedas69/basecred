...
import { fetchFarcasterScore } from '../repositories/farcaster.repository.js';
 ...
 */
function computeRecency(ethos, talent, farcaster) {
    const sources = [];
 ...
 */
function buildFarcasterFacet(result, config) {
...
export async function getUnifiedProfile(address, config) {
    ...
    const [ethosSettled, talentSettled, farcasterSettled] = await Promise.allSettled([
        ...
        farcasterEnabled
            ...
            : Promise.resolve({ availability: 'not_requested' }),
    ]);
