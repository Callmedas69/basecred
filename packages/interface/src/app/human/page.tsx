...
import { ONCHAIN_CONTRACTS, BASE_SEPOLIA_CHAIN_ID } from "@/lib/onChainContracts";
 ...
 */
async function subjectToBytes32Browser(subject: string): Promise<`0x${string}`> {
...
export default function HumanPage() {
  ...
  const [submitPhases, setSubmitPhases] = useState<Map<string, "wallet" | "confirming">>(new Map());
  const [submitResults, setSubmitResults] = useState<Map<string, { txHash?: string; error?: string }>>(new Map());
  const [pendingTx, setPendingTx] = useState<{ ctx: string; hash: `0x${string}` } | null>(null);
  ...
  const { data: txReceipt, error: txReceiptError } = useWaitForTransactionReceipt({
    hash: pendingTx?.hash,
  ...
  useEffect(() => {
    ...
    if (txReceipt) {
      ...
      setSubmitPhases((prev) => { const n = new Map(prev); n.delete(ctx); return n; });
      setPendingTx(null);
    }
    ...
    if (txReceiptError) {
      setSubmitResults((prev) => {
        const next = new Map(prev);
        next.set(ctx, { error: txReceiptError.message || "Failed to confirm transaction" });
        return next;
      });
      setSubmitPhases((prev) => { const n = new Map(prev); n.delete(ctx); return n; });
      setPendingTx(null);
  ...
  const evaluateContext = useCallback(async (ctx: string, walletAddress: string) => {
    ...
    setContextResults((prev) => {
      ...
      const next = new Map(prev);
      next.set(ctx, { context: ctx, decision: null, proof: null, loading: true, proofLoading: false, error: null });
      return next;
    });
    ...
    try {
      ...
      setContextResults((prev) => {
        const next = new Map(prev);
        next.set(ctx, { context: ctx, decision, proof, loading: false, proofLoading: false, error: null });
        return next;
      });
    } catch (err: any) {
      const message =
      ...
      setContextResults((prev) => {
        const next = new Map(prev);
        next.set(ctx, { context: ctx, decision: null, proof: null, loading: false, proofLoading: false, error: message });
        return next;
      });
    } finally {
  ...
  const currentContextResult = selectedContext ? contextResults.get(selectedContext) : null;
  ...
  const handleSubmitOnChain = async (ctx: string) => {
    ...
    try {
      ...
      const decisionValue = encodeDecision(ctxResult.decision.decision as Decision);
      ...
      <div className="max-w-5xl mx-auto space-y-8 md:space-y-12 pt-24 md:pt-28 lg:pt-32 px-4 sm:px-6 md:px-12 pb-8 sm:pb-12 md:pb-16">
        ...
        <div className="space-y-3 md:space-y-4 text-center md:text-left">
          ...
          <p className="text-muted-foreground text-base sm:text-lg md:text-xl max-w-2xl leading-relaxed pl-2 border-l-4 border-teal-500">
            {isConnected
              ? "See your on-chain reputation and submit verified decisions."
            ...
            <div className="max-w-md mx-auto space-y-6 md:space-y-8 px-4">
              ...
              <div className="space-y-3 md:space-y-4">
                ...
                <p className="text-sm sm:text-base text-muted-foreground/80 leading-relaxed">
                  Connect your wallet to see your normalized signals, eligibility status, and submit
                  verified decisions on-chain.
                </p>
              </div>
              ...
              <div className="max-w-md mx-auto space-y-6 px-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm sm:text-base text-muted-foreground">
                    Generating a zero-knowledge proof for <span className="font-mono text-foreground">{selectedContext}</span>.
        ...
        {!contextsLoading && isConnected && !isContextLoading && currentDecision && currentProof && (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-6 md:space-y-8">
              ...
              <div className="relative z-10 p-6 sm:p-8 md:p-12">
                <div className="space-y-3 md:space-y-4">
                  <div className="font-mono text-xs sm:text-sm tracking-widest text-gray-500 flex items-center gap-2">
                    <span className="opacity-50">&gt;_</span> REPUTATION DECISION
                  </div>
                  <div
                    className={cn(
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2">
                    <span
                      className={cn(
                        ...
                        DECISION_LABELS[currentDecision.decision]?.color || "text-foreground"
                      )}
                    >
                      <span className="w-1.5 h-1.5 rounded-full bg-current" />
                      {DECISION_LABELS[currentDecision.decision]?.label || currentDecision.decision}
                    </span>
                    ...
                    {submitResults.get(selectedContext)?.txHash ? (
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 text-emerald-600">
                          ...
                          <span className="text-sm font-medium">Confirmed on-chain</span>
                        </div>
                        <a
                      ...
                      <div className="text-sm text-amber-600 flex items-center gap-2">
                        ...
                        Contract is restricted. Your wallet is not authorized to submit.
                      </div>
                    ) : isWrongChain ? (
                      <Button
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="flex flex-wrap gap-2 sm:gap-4">
                ...
                <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 bg-muted rounded-md border border-border text-xs sm:text-sm font-mono text-foreground font-bold uppercase tracking-wider">
                  ...
                  COVERAGE: {Math.round((currentDecision.signals?.signalCoverage || 0) * 100)}%
                </div>
              </div>
              <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-muted-foreground leading-relaxed max-w-3xl">
                ...
                trust signals with{" "}
                <span className="text-foreground font-medium">
                  {formatSignal(currentDecision.signals?.spamRisk).toLowerCase()}
                </span>{" "}
            ...
            <Tabs defaultValue="structured" className="w-full">
              ...
              {/* Signals Tab */}
              <TabsContent value="structured" className="space-y-8 md:space-y-12">
                <div className="grid sm:grid-cols-2 gap-x-6 md:gap-x-12 gap-y-0 font-mono text-sm md:text-base border-t border-border pt-3 sm:pt-4">
                  ...
                  <div className="flex justify-between items-center py-2.5 sm:py-3 border-b border-border/50">
                    ...
                    <span className="font-bold text-foreground text-xs sm:text-sm md:text-base">
                      {Math.round((currentDecision.signals?.signalCoverage || 0) * 100)}%
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2.5 sm:py-3 border-b border-border/50">
                    <span className="text-muted-foreground text-xs sm:text-sm md:text-base">
                ...
                <div className="space-y-3 md:space-y-4 pt-3 sm:pt-4">
                  ...
                  <div className="bg-muted/30 rounded-lg p-4 sm:p-6 space-y-2 border border-border">
                    {currentDecision.explain.map((reason: string, i: number) => (
                      <div key={i} className="flex gap-2 sm:gap-3 text-muted-foreground">
                        <div className="w-1.5 h-1.5 rounded-full bg-teal-500 mt-1.5 sm:mt-2 shrink-0 shadow-[0_0_10px_rgba(20,184,166,0.5)]" />
                        <span className="font-mono text-xs sm:text-sm break-words">{reason}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-muted/30 rounded-lg p-4 border border-border space-y-1">
                        <div className="flex justify-between text-xs sm:text-sm font-mono">
                          <span className="text-muted-foreground">policyHash:</span>
                        ...
                        <div className="flex justify-between text-xs sm:text-sm font-mono">
                          ...
                          <span className="text-foreground">{currentProof.publicSignals[1]}</span>
                        </div>
                        <div className="flex justify-between text-xs sm:text-sm font-mono">
                          <span className="text-muted-foreground">decision:</span>
                          <span className="text-foreground">{currentProof.publicSignals[2]}</span>
                        </div>
                      </div>
                    </div>
        ...
        {!contextsLoading && isConnected && !isContextLoading && !currentDecision && currentContextResult?.error && (
          <div className="animate-in fade-in space-y-6 md:space-y-8">
            ...
            <div className="py-12 text-center border-2 border-dashed border-border/50 rounded-2xl bg-muted/10">
              <div className="max-w-md mx-auto space-y-4 px-4">
                ...
                <h2 className="text-xl font-bold text-muted-foreground">Evaluation Failed</h2>
        ...
        {!contextsLoading && isConnected && contexts.length === 0 && (
          <div className="animate-in fade-in py-12 text-center border-2 border-dashed border-border/50 rounded-2xl bg-muted/10">
            <div className="max-w-md mx-auto space-y-4 px-4">
              <AlertCircle className="w-12 h-12 mx-auto text-muted-foreground" />
              <h2 className="text-xl font-bold text-muted-foreground">No Contexts Available</h2>
              <p className="text-sm text-muted-foreground">
                Could not load decision contexts. Please check the API configuration.
              </p>
            </div>
          </div>
        )}
